...
Protokolün hesaplama çekirdeği TenSEAL \cite{benaissa2021tenseal} ile yeniden kodlanmıştır: CKKS, $N = 16384$ ve $\Delta = 2^{40}$ makaleyle aynıdır; modül zinciri ise bir seviye kısa, $[60, 40, 40, 40, 60]$ olarak seçilmiştir (Bölüm \ref{sec:ckks}). Hesaplama iki lineer turdan oluşur: $M_1 \in \mathbb{R}^{20 \times n}$ ve $M_2 \in \mathbb{R}^{10 \times 20}$; ağırlıklar rastgele üretilmiştir. Şifreli ve açık katkılar lineerlik sayesinde tur boyunca ayrı tutulur ve istemci sonunda birleştirir. Tam homomorfik şifreleme durumu, ROI'nin tüm görüntüyü kapsadığı özel durum olarak ele alınmıştır.

Bir şifreli metin en fazla $8192$ değer taşıdığından pikseller şifreli metin başına $8192$ değerlik dilimler hâlinde yoğun paketlenmiştir; örneğin $512 \times 512$ görüntünün tam şifrelenmesi 32 şifreli metin gerektirir. Ön denemelerde TenSEAL'in slot toplama işleminin 2'nin kuvveti olmayan vektör uzunluklarında belirgin biçimde yavaşladığı gözlenmiştir; bu durumda ROI şifrelemek tüm görüntüyü şifrelemekten yavaş çıkabilmektedir. Karşılaştırmanın adil olması için her dilim sıfırlarla en yakın 2'nin kuvvetine tamamlanmıştır. Sıfır değerli elemanlar iç çarpımın sonucunu değiştirmez.

\textbf{Doğruluk.} Seçici şifreli çıktının şifresiz hesaplamayla aynı olduğu, her veri kümesinden bir örnek görüntü üzerinde beyin MR görüntüsünde tümör maskesi ve göğüs röntgeninde akciğer maskesi kullanılarak $64 \times 64$ ve $128 \times 128$ çözünürlüklerde doğrulanmıştır.

\textbf{Maliyet.} $64$, $128$, $256$ ve $512$ piksel kenar uzunluklu görüntülerde $\rho \in \{0.01, 0.05, 0.10, 0.25, 0.50, 0.75, 0.90, 1.00\}$ şifreli alan oranları için istemci şifreleme süresi, sunucu hesaplama süresi, çözme süresi, şifreli metin sayısı ve istemciden sunucuya giden veri boyutu ölçülmüştür. Ölçümler bilgisayarda başka iş çalışmıyorken yapılmış; $64$, $128$ ve $256$ piksel görüntülerde 5, $512$ piksel görüntülerde 3 kez tekrarlanarak ortalaması alınmıştır (makalede 10 tekrar). Hız kazancı, aynı görüntü boyutunda tam şifrelemenin toplam süresinin seçici şifrelemenin toplam süresine oranı olarak tanımlanmıştır.
...
\textbf{Kaggle göğüs röntgeni kümesi.} Kaggle'da yayımlanmış, COVID-19, normal ve pnömoni sınıflarından 6.432 görüntü içeren bir derleme kullanılmıştır \cite{kaggle2020cxr}. Normal ve pnömoni görüntüleri çocuk hastalara aittir ve görüntüler orijinal çözünürlüklerini korumaktadır.
...
\textbf{Önişleme.} CNN tabanlı deneylerde görüntüler gri tonlamalı $224 \times 224$ çözünürlüğe bilineer, maskeler en yakın komşu yöntemiyle indirilmiştir. COVID-QU-Ex ve Kaggle görüntüleri dağıtıldıkları 8 bitlik biçimde kullanılmıştır. Beyin MR kesitlerinin ham yoğunlukları görüntü başına $0.5$ ve $99.5$ yüzdelikleri arasında $[0, 1]$ aralığına ölçeklenmiştir. Bağlam saldırısı, kök neden, savunma ve bozuk bağlam deneylerinde bu yüzdelikler \emph{yalnızca sunucunun gördüğü piksellerden} hesaplanmıştır; gizli bölgenin yoğunlukları normalizasyona girmez (Bölüm \ref{sec:attackB}). Temsilin tamamının şifrelendiği FoveaHE ve şifreli özet deneylerinde ölçekleme istemcide kesitin tüm pikselleriyle yapılmıştır.
...
\textbf{Beyin MR normalizasyonu.} Beyin MR kesitlerinin yoğunlukları için iki normalizasyon düzeni karşılaştırılmıştır. \emph{Kesit tabanlı normalizasyon}, yüzdelikleri kesitin tüm pikselleriyle (tümör dahil) hesaplar. \emph{Görünür piksel normalizasyonu} ise yüzdelikleri, görüş oluşturulurken ve artırmadan sonra, yalnızca sunucunun gördüğü piksellerden hesaplar; gizli bölge ölçeğe hiç girmez. Kesit tabanlı düzende gizli tümörün parlaklığı ölçek üzerinden açık piksellere taşınabilir; bu, gerçek sunucunun göremeyeceği yapay bir kanal olabileceğinden bağlam saldırısı, kök neden, savunma ve bozuk bağlam deneyleri asıl olarak görünür piksel normalizasyonuyla yapılmıştır. İki düzenin sonuçları Bölüm \ref{sec:res:B}'te karşılaştırılmıştır. Kanalın tek başına taşıdığı bilgi de ölçülmüştür: her kesitte tüm piksellerden ve yalnızca tümör dışı piksellerden hesaplanan yüzdelik aralıklarının oranıyla, Saldırı A'daki sınıflandırıcı ve katlarla tümör tipi tahmin edilmiştir.

\textbf{Değerlendirme hassasiyeti.} Geliştirme sırasında önemli bir yöntemsel hata tespit edilmiştir. Tüm girdilerin özdeş olduğu tamamen gizli görüşte AUC'nin tam $0.5$ olması gerekirken 16 bit hassasiyetle ve sınıflara göre sıralı test indeksleriyle yapılan değerlendirmede $0.421$ bulunmuştur. Bunun nedeni, yığın bileşimine bağlı çok küçük sayısal farkların sınıf sırasıyla hizalanmasıdır. Bu nedenle tüm tahminler 32 bit hassasiyetle ve karışık sırada hesaplanmış, düzeltilmiş boru hattında tamamen gizli görüşün AUC'si her iki veri kümesinde $0.500$ olarak doğrulanmıştır. Ekran kartının hesaplama yolu yığın boyutuna göre değişebildiğinden, son değerlendirme yığını da ilk örneğin tekrarıyla sabit boyuta tamamlanmış ve fazlası atılmıştır. Bu kontrol savunma ve çözüm deneylerine kalıcı bir akıl sağlığı testi olarak eklenmiştir.
...
Katmanlar pencere içinde alan ortalamalı örneklemeyle (RoIAlign, uyarlanabilir örnek ızgarası) hesaplanır ve 8 bitlik tam sayılara nicelenir. Şifrelenen vektör $[\text{odak}, \text{genel bakış}, \text{geometri}]$ biçimindedir ve $F^2 + G^2 + 3$ değer içerir. Yapılandırmalar bu boyutlarla adlandırılmıştır: F32\_G16 $32^2 + 16^2 + 3 = 1.283$ değer, F64\_G32 ise $5.123$ değerdir. Her ikisi de $8.192$ slotluk tek bir şifreli metne sığar. Karşılaştırma için tam görüntü beyin MR verisinde $262.144$, göğüs röntgeninde $65.536$ değerdir. ROI'nin hemen çevresindeki kitle etkisini daha yüksek çözünürlükte taşıyabileceği düşüncesiyle, odakla aynı merkezli ve 2 ya da 3 kat büyük pencereden alınan bir ``yakın çevre'' katmanı (P) da denenmiştir. Bilgi üst sınırı taramasında (Bölüm \ref{sec:fov:info}) yakın çevre eklemek, aynı odak ve genel bakışa sahip yapılandırmaya göre AUC'yi beyin MR verisinde ortalama $0.001$ düşürmüş ($-0.004$ ile $+0.001$ arası), göğüs röntgeninde değiştirmemiştir. Bu nedenle yakın çevre katmanı ana yapılandırmalarda kullanılmamıştır. Şekil \ref{fig:temsil} ana yapılandırmaların katmanlarını örneklerle göstermektedir.

\begin{figure}[h!]
 \centering
 \includegraphics[width=\textwidth,keepaspectratio=true]{./fig/cozum_temsil_ornek_brain_tez.png}
 \caption{Beyin MR verisinde F64\_G32 yapılandırmasıyla odaklı temsil örnekleri (satırlar: meningiom, gliom, hipofiz tümörü). Sütunlar soldan sağa: odak penceresiyle (mavi kare) tam görüntü, $64 \times 64$ odak, $32 \times 32$ genel bakış, bu katmanlardan geri çizilen $224 \times 224$ görüntü ve karşılaştırma için eş örnekli küçültme (U64).}
 \label{fig:temsil}
\end{figure}

\textbf{Referans temsiller.} Odaklamanın katkısını ayırmak için üç referans kullanılmıştır. \emph{Eş örnekli küçültme} (U$G$), tüm görüntünün yalnızca $G \times G$ örneğidir ve aynı değer bütçesinde odaksız şifrelemeyi temsil eder; örneğin U64 $4.096$ değerdir. \emph{ROI penceresi referansı}, bilgi üst sınırı deneyinde görüntüye pencerenin konumunu gösteren ek bir kanal ekler. \emph{Eş örnekli küçültme + geometri} (U64 + geometri, $4.099$ değer, tek şifreli metin) ise şifreli çalışabilen modellerde U64'e odaklı temsilin üç geometri değerini ekler. Son iki referans modele ROI'nin konum ve boyut bilgisini FoveaHE ile aynı ölçüde verir; böylece odaklı temsilin kazancının ROI konum bilgisinden mi, yoksa çözünürlüğün odağa verilmesinden mi geldiği ayrılabilir.
...
Model D, D2 ve C (Bölüm \ref{sec:hemodels}), odaklı temsillerde, özgün çözünürlükteki tam görüntüde, U64'te ve U64 + geometri referansında aynı düzenle eğitilmiştir. U64 + geometri referansında geometri Model D ve D2'de girdiye eklenmiş, Model C'de standartlaştırılıp tam bağlantılı katmana verilmiştir. Tam görüntü üzerindeki Model D, $\Pi_{\mathrm{ROI}}$ protokolünün hesaplama yapısıyla aynıdır. Seçici şifreleme model çıktısını değiştirmediği için bu modelin başarısı $\Pi_{\mathrm{ROI}}$'nin teşhis başarısını temsil eder.
...
Sonuçlara göre hedef kaydırılmaması için FoveaHE'nin başarı ölçütleri 15 Eylül 2026'da, çözüm deneyleri başlamadan önce çalışma planına yazılmış ve sonradan değiştirilmemiştir (Çizelge \ref{tbl:olcutler}). Ölçütlerin dondurulduğu çalışma planının tarihli bir kopyası kod deposunda \texttt{docs/calisma\_plani\_2026-09-15.md} olarak bulunur. Bunlardan bilgi kaybı, model kaybı ve şifreli doğruluk eşikleri ilk sonuçlardan önce deney betikleriyle birlikte kod deposuna da işlenmiştir (commit \texttt{1be0d66}, \texttt{edf0933} ve \texttt{868002d}; ilk sonuç commit'i \texttt{803a90c}, 15 Eylül 2026 21:46). Sızıntı ve hız eşikleri ise depoya ilk kez sonuçlarla birlikte kaydedilmiş olduğundan, bu iki eşiğin önceden belirlendiği söz konusu çalışma planı belgesiyle belgelenmektedir. Ana yapılandırmalarda doğruluk deneyleri 5 tohumla tekrarlanmış, güven aralıkları önyüklemeyle (beyin MR verisinde hasta düzeyinde) hesaplanmıştır.
